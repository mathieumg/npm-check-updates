...
function analyzeProjectDependencies(pkgData, pkgFile) {
    var pkg = jju.parse(pkgData);
    var jsonpkgData = jju.stringify(pkg, {mode: 'json'});
    var current = vm.getCurrentDependencies(jju.parse(pkgData, {mode: 'json'}), {
        prod: program.prod,
        dev: program.dev,
    ...
    .spread(function (current, installed, upgraded, latest) {
        ...
        if(program.json) {
            var newPkgData = vm.updatePackageData(jsonpkgData, current, upgraded);
            ...
            if(pkgFile && !_.isEmpty(upgraded)) {
                if (program.upgrade) {
                    var newPkgData = vm.updatePackageData(jsonpkgData, current, upgraded);
                    newPkgData = jju.update(pkgData, JSON.parse(newPkgData));
                    writePackageFile(pkgFile, newPkgData)
                        .then(function () {
                            print('\n' + pkgFile + " upgraded");
                        });
                } else {
                    print("Run with '-u' to upgrade your package");
                }
...
function programRunLocal() {
    ...
    if(!process.stdin.isTTY) {
        ...
        pkgFile = closestPackage.sync('json', process.cwd());
        if (!fs.existsSync(pkgFile)) {
            pkgFile = closestPackage.sync('json5', process.cwd());
            if (!fs.existsSync(pkgFile)) {
                throw new Error('package.json5 or package.json not found');
            }
        }
        ...
        if(
            relPathToPackage !== 'package.json' &&
            relPathToPackage !== 'package.json5'
        ) {
